...
All config options will be placed in config.yaml. You can specify the snap file name, this could be used in a situation where you have a large amount of config files.  You can set the headless browser, domains of the 2 sites you are comparing, URL paths, screen widths & HTTP headers.  Instead of setting paths, you may use the spidering option.  This requires removing/commenting out the `path:` entries in `config.yaml`.  We use ImageMagick to process the images, we set a fuzz on the images to handle anti aliasing, this can be lowered or increased in the config.
